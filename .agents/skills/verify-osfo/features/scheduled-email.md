...
3. Start Scheduled Email evidence. Run `control-osfo scheduled-email-request <run-id>` once; this sends the strict run-owned request through the linked Agent rather than calling the service directly.
...
6. Continue to [account deletion](account-deletion.md) so run-owned Integration and Workflow state is removed.
