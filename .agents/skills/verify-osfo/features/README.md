...
- [Read chat attachments and fill a PDF](chat-pdf-form.md) prepares synthetic image/form bytes and checks the actual downloaded PDF. This drive remains draft until the connected attachment tools, ordinary download link, and committed authority/replay/deletion evidence are qualified on the integrated build.
- [Send an immediate Gmail message](immediate-gmail-send.md) proves one exact authenticated Approval, current authority before the external effect, one local provider application, visible terminal outcome, once-only Gmail accounting, rejected Approval replay, and exactly-once retained-request revocation and deletion of the captured Gmail Connection while an unrelated `ACTIVE` Connection and the immutable send ledger remain unchanged.
