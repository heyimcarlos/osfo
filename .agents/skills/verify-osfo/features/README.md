...
- [Send an immediate Gmail message](immediate-gmail-send.md) preserves proof of one exact authenticated Approval, current authority before the external effect, one local provider application, visible terminal outcome, once-only Gmail accounting, rejected Approval replay, and later retained-request account-deletion replay with User and Agent Directory absence. Its outcome remains `MISSING` until #187 supplies provider Connection deletion proof.
...
- `MISSING` means the recorded evidence passed every delivered local check, but a named required proof is not implemented. It never qualifies a feature for release.
