...
- [Send an immediate Gmail message](immediate-gmail-send.md) proves one exact authenticated Approval, current authority before the external effect, one local provider application, visible terminal outcome, once-only Gmail accounting, rejected replay, and later deletion of its retained Agent facet.
