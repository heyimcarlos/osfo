...
func run(ctx context.Context) error {
	flags := flag.NewFlagSet("b2-load", flag.ContinueOnError)
	url := flags.String("url", "", "authenticated B2 ingress URL")
	...
	ordering := flags.String("ordering", string(b2.DatabaseFirst), "dual-write ordering")
	...
	for ordinal := 0; ordinal < *count; ordinal++ {
		...
		go func(ordinal int, scheduledAt time.Time) {
			...
			samples <- offer(ctx, client, *url, *accessToken, benchmarkID, ordinal, b2.Ordering(*ordering), scheduledAt)
		}(ordinal, scheduledAt)
	}
	wait.Wait()
	close(samples)
...
func offer(ctx context.Context, client *http.Client, url, accessToken string, benchmarkID uuid.UUID, ordinal int, ordering b2.Ordering, scheduledAt time.Time) sample {
	offeredAt := time.Now().UTC()
	identity := fmt.Sprintf("%s/%d", benchmarkID, ordinal)
	requestBody := b2.Request{
		BenchmarkID: benchmarkID, Ordinal: ordinal, Attempt: 1,
		Idempotency: "b2/" + identity, RequestHash: "sha256:b2/" + identity,
		Ordering: ordering, Fault: b2.NoFault,
	}
