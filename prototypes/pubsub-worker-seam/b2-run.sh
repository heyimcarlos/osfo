...
state_file="$prototype_dir/.b2-run.env"
evidence_root="$prototype_dir/evidence/b2-negative-control"

project_id=${GCP_PROJECT_ID:-$(gcloud config get-value project 2>/dev/null)}
region=${GCP_REGION:-northamerica-northeast1}
prefix=${RESOURCE_PREFIX:-osfo-b2-35}
...
load_state() {
  if [[ ! -f "$state_file" ]]; then
    echo "Missing $state_file. Run ./b2-run.sh provision first." >&2
    exit 1
  fi
...
provision() {
  ...
  bind_project_role "$ingress_service_account" roles/pubsub.publisher
  ...
  for account in "$worker_service_account" "$ingress_service_account"; do
  ...
  gcloud run deploy "$ingress_service" --image="$image_uri" --command=/b2-ingress \
    ...
    --set-secrets="DATABASE_URL=$database_secret:latest" \
    --set-env-vars="GCP_PROJECT_ID=$project_id,PUBSUB_TOPIC_ID=$topic_id,DB_POOL_SIZE=8"
  ...
  start_proxy
  (cd "$prototype_dir" && DATABASE_URL=$(local_database_url) go run ./cmd/b2-harness migrate)
...
run_cut_matrix() {
  ...
  (cd "$prototype_dir" && \
    ...
    go run ./cmd/b2-harness matrix --repetitions=100 --seeds=3 --drain=60s \
...
hard_crash_smoke() {
  ...
  mkdir -p "$destination"
  local ordering fault precedence benchmark_id lane
  for ordering in database_first publish_first concurrent; do
    for precedence in database publish; do
      if [[ "$precedence" == database ]]; then
        fault=after_database_commit
      else
        fault=after_publish_confirmation
      fi
      lane="hard-crash/$ordering/$precedence"
      benchmark_id=$(uuidgen | tr '[:upper:]' '[:lower:]')
      (cd "$prototype_dir" && DATABASE_URL=$(local_database_url) go run ./cmd/b2-harness prepare \
        --benchmark="$benchmark_id" --lane="$lane" --expected-incoming=1)
      local delay_flags=()
      if [[ "$ordering" == concurrent && "$precedence" == database ]]; then
        delay_flags+=(--publish-delay=2s)
      fi
      if [[ "$ordering" == concurrent && "$precedence" == publish ]]; then
        delay_flags+=(--database-delay=2s)
      fi
      set +e
      (cd "$prototype_dir" && DATABASE_URL=$(local_database_url) GCP_PROJECT_ID="$project_id" PUBSUB_TOPIC_ID="$topic_id" \
        go run ./cmd/b2-harness admit --benchmark="$benchmark_id" --ordinal=0 --attempt=1 \
          --ordering="$ordering" --fault="$fault" --hard-crash "${delay_flags[@]}") \
        >"$destination/$ordering-$precedence.stdout" 2>"$destination/$ordering-$precedence.stderr"
      local exit_code=$?
      set -e
      printf '%s\n' "$exit_code" >"$destination/$ordering-$precedence.exit-code"
      sleep 20
      (cd "$prototype_dir" && DATABASE_URL=$(local_database_url) go run ./cmd/b2-harness audit \
        --benchmark="$benchmark_id" --expected-incoming=1) >"$destination/$ordering-$precedence.audit.json"
    done
  done
  seal_directory "$destination"
}
...
load_lane() {
  ...
  local ordering=${5:-database_first}
  ...
  local started_at
  started_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)
  (cd "$prototype_dir" && GCP_IDENTITY_TOKEN="$identity_token" go run ./cmd/b2-load \
    --url="$ingress_url" --benchmark="$benchmark_id" --rate="$rate" --end-rate="$end_rate" \
    --duration="${duration}s" --count="$count" --ordering="$ordering" \
  ...
  jq -n --arg benchmark_id "$benchmark_id" --arg lane "$lane" --arg started_at "$started_at" \
    --arg offer_ended_at "$offer_ended_at" --arg ended_at "$ended_at" --arg ordering "$ordering" \
    --argjson rate "$rate" --argjson end_rate "$end_rate" --argjson duration "$duration" \
    --argjson count "$count" --argjson repetition "$repetition" \
    '{manifest:"pubsub-handoff-v1",benchmark_id:$benchmark_id,lane:$lane,repetition:$repetition,ordering:$ordering,rate_per_second:$rate,end_rate_per_second:$end_rate,duration_seconds:$duration,count:$count,started_at:$started_at,offer_ended_at:$offer_ended_at,ended_at:$ended_at}' \
    >"$destination/scenario.json"
  capture_logs "$destination/runtime-logs.json" "$started_at"
...
load_manifest() {
  ...
  load_lane linear-ramp 23 900 1 database_first 464
}
...
finalize_interrupted_lane() {
  ...
  jq -n --arg benchmark_id "$benchmark_id" --arg lane "$lane" --arg started_at "$started_at" \
    ...
    '{manifest:"pubsub-handoff-v1",benchmark_id:$benchmark_id,lane:$lane,repetition:1,ordering:"database_first",rate_per_second:$rate,end_rate_per_second:$rate,duration_seconds:$duration,count:$count,started_at:$started_at,offer_ended_at:$offer_ended_at,ended_at:$ended_at,audit_recovered_after_controller_interrupt:true}' \
    >"$destination/scenario.json"
  capture_logs "$destination/runtime-logs.json" "$started_at"
  collect_monitoring "$destination" "$started_at" "$ended_at"
  gzip -9 "$destination/caller-samples.jsonl" "$destination/runtime-logs.json"
...
capture_logs() {
  local destination=$1
  local started_at=$2
  gcloud logging read "timestamp>=\"$started_at\" AND (resource.labels.service_name=\"$worker_service\" OR resource.labels.service_name=\"$ingress_service\")" \
...
collect_monitoring() {
  ...
  for service in "$worker_service" "$ingress_service"; do
    for metric in \
...
usage() {
  echo "Usage: ./b2-run.sh provision|deploy|auth-smoke|hard-crash-smoke|cut-matrix|load <lane> <rate> <seconds> [repetition] [ordering] [end-rate]|finalize <directory> <benchmark> <expected> <rate> <seconds> <lane>|load-manifest|scale-zero|inventory|seal|teardown|decision"
}
