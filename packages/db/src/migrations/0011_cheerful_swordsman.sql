CREATE TABLE "account_deletion_actions" (
	...
	"replay_token_hash" text NOT NULL,
	...
	CONSTRAINT "account_deletion_actions_identity_check" CHECK (length(btrim("account_deletion_actions"."action_id")) > 0 and length(btrim("account_deletion_actions"."user_id")) > 0 and length(btrim("account_deletion_actions"."auth_session_id")) > 0 and "account_deletion_actions"."replay_token_hash" ~ '^[0-9a-f]{64}$'),
