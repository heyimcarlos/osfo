...
import { assessRecovery } from "../src/qualification/recovery-evidence";
import { assessResourceHeadroom } from "../src/qualification/resource-headroom";
import { generateOpenArrivals } from "../src/qualification/workload-generation";
import { manifestVersions } from "./support/qualification-fixtures";
...
describe("Production qualification harness", () => {
  ...
  it("requires measured 30 percent headroom and rejects duplicate evidence", () => {
    ...
    const measurements = Array.from({ length: 3 }, (_, repetition) => ({
      ...
      runArtifactChecksum: "run-checksum",
      unit: "queries",
    }));
