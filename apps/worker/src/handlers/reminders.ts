...
import { Api, CurrentUser, RemindersUnavailable, type CurrentUserValue } from "@osfo/api";
...
/** Expose exact pending Approval decisions for Reminders. */
export const layer = (bindings: Bindings) =>
  Layer.unwrap(
    Effect.gen(function* () {
      ...
      return HttpApiBuilder.group(Api, "reminders", (handlers) =>
        handlers
          .handle("approvals", () =>
...
const withAgent = <Value>(
  ...
  ) => Effect.Effect<Value, RemindersUnavailable>,
) =>
  Effect.gen(function* () {
...
export const listApprovals = (
  ...
  rpc(
    () => stub.listActionPresentations(agentId, actorFor(currentUser), "reminder"),
    ActionPresentationsFound,
  ).pipe(
...
const unavailable = () =>
  new RemindersUnavailable({
    message: "Reminder controls are temporarily unavailable. Please try again.",
  });

export * as ReminderHandlers from "./reminders";
