...
import { Api, CurrentUser, BrowserApprovalsUnavailable, type CurrentUserValue } from "@osfo/api";
...
/** Expose exact pending Approval decisions for browser interactions. */
export const layer = (bindings: Bindings) =>
  Layer.unwrap(
    Effect.gen(function* () {
      ...
      return HttpApiBuilder.group(Api, "browserApprovals", (handlers) =>
        handlers
          .handle("approvals", () =>
...
const withAgent = <Value>(
  ...
  ) => Effect.Effect<Value, BrowserApprovalsUnavailable>,
) =>
  Effect.gen(function* () {
...
export const listApprovals = (
  ...
  rpc(
    () => stub.listActionPresentations(agentId, actorFor(currentUser), "browser"),
    ActionPresentationsFound,
  ).pipe(
      ...
      items: presentations
        .filter(
          ({ actionDefinitionVersion, operation }) =>
            operation === "browser.effect" && actionDefinitionVersion === "osfo-browser-effect-v1",
        )
...
const unavailable = () =>
  new BrowserApprovalsUnavailable({
    message: "browser interaction controls are temporarily unavailable. Please try again.",
  });

export * as BrowserApprovalHandlers from "./browser-approvals";
