...
export interface CommittedTurnWindow {
  ...
  readonly observedAtOrAfter: string;
...
export const makeAgentStore = (db: AgentDb) => {
  ...
  const readCommittedTurnWindow = (window: CommittedTurnWindow) =>
    execute("readCommittedTurnWindow", () =>
      db
        .select(committedTurnReceiptFields)
        .from(committedTurns)
        .where(
          and(
            gte(committedTurns.observed_at, window.observedAtOrAfter),
      ...
      Effect.flatMap((rows) =>
        Effect.forEach(rows, (row) => decodeCommittedTurnReceipt("readCommittedTurnWindow", row)),
      ),
      Effect.map((rows) =>
