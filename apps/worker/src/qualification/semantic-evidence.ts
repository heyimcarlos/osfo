...
/** Acceptance fact and semantic identities read from its committed Agent SQLite row. */
export interface AgentSqliteProductEvidence {
  readonly acceptanceReceiptId: string;
  readonly authority: "osfo_acceptance_receipts";
...
export const LocalProductEvidenceBoundary = Schema.Union([
  Schema.Struct({
    ...
    authority: Schema.Literal("osfo_acceptance_receipts"),
  ...
  for (const rootId of input.acceptedRootIds) {
    ...
    for (const store of requirements.requiredStores) {
      ...
      for (const record of records) {
        ...
        if (
          (record.store === "AgentSQLite" &&
            (record.authority !== "osfo_acceptance_receipts" ||
              record.productFactId !== record.acceptanceReceiptId ||
              record.thinkSubmissionId !== trace.correlations.thinkSubmissionId)) ||
    ...
    for (const kind of requiredOperationKinds) {
      ...
      const sorted = [...operation.samples].sort((left, right) => left - right);
      if (
        operations.length !== 1 ||
        sorted.length === 0 ||
