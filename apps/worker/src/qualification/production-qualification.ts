...
export const qualifyProduction = (
      ...
      [...outcome.assertions, ...outcome.milestoneAssertions].every(
        (assertion) =>
          qualificationChecksum(
            EffectArray.sortWith(assertion.authorityFactIds, (id) => id, Order.String),
          ) ===
          qualificationChecksum(EffectArray.sortWith(authorityFactIds, (id) => id, Order.String)),
      );
    if (
  ...
  const derivedGoodRootIds = EffectArray.sortWith(
  ...
  if (
    ...
    qualificationChecksum(
      EffectArray.sortWith(evidence.cost.goodRootOutcomeIds, (rootId) => rootId, Order.String),
    )
  ) {
    findings.push(
      finding(
    ...
    if (record === undefined) {
      ...
      record.usage.some(
        (line) =>
          !trace.signals.some(
            (signal) =>
              signal.component === costCategoryAuthorities[line.category] &&
              signal.signalId === line.sourceProductFactId,
          ) ||
          (!["PostgreSQL", "R2"].includes(costCategoryAuthorities[line.category]) &&
            !authorityUsageFacts.some(
              (fact) =>
                fact.rootId === trace.rootId &&
