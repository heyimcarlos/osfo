...
const systemSkills = [
  ...
  },
  {
    ...
    instructions: [
      ...
      "Use exportDocument only for an already retained document owned by the current User. After generateDocument or exportDocument succeeds, include the exact returned downloadUrl in your reply so the User can open and download it. Never construct or alter a download URL. Explain that the link requires sign-in to the same Osfo account.",
