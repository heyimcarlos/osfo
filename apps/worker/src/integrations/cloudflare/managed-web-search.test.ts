...
const response = {
  id: "response-1", max_output_tokens: 512, max_tool_calls: 1,
  output: [call, {
    content: [{ annotations: [{ title: "Source title", type: "url_citation", url: source }], text: "Untrusted generated answer", type: "output_text" }], type: "message",
  }], status: "completed", tools: [{ type: "web_search" }],
  usage: { input_tokens: 8541, input_tokens_details: { cached_tokens: 0 }, output_tokens: 91 },
};

describe("managed public search", () => {
  it.effect("rates one real search including its input tokens exactly once", () => Effect.gen(function* () {
    ...
    expect(decoded.evidence.executedSearches).toEqual([{ errorCode: null, outcome: "succeeded", query: "example query", toolCallId: "search-1" }]);
  }));
  it.effect("retains unknown cost rather than inventing zero for malformed response", () => Effect.gen(function* () {
    const result = yield* Effect.result(decodeResponse({ output: [] }, initial));
    expect(Result.isFailure(result)).toBe(true);
    if (Result.isFailure(result)) expect(result.failure.managedSearch?.ratedCostUsdMicros).toBeNull();
  }));
  it.effect("refuses answer-only output while retaining paid model usage", () => Effect.gen(function* () {
  ...
  it.effect("refuses multiple search calls without understating ambiguous search cost", () => Effect.gen(function* () {
    const result = yield* Effect.result(decodeResponse({ ...response, output: [call, { ...call, id: "search-2" }] }, initial));
    expect(Result.isFailure(result)).toBe(true);
    if (Result.isFailure(result)) expect(result.failure.managedSearch?.ratedCostUsdMicros).toBeNull();
  }));
  it.effect("never retries an ambiguous paid request", () => Effect.gen(function* () {
    const requests: Array<unknown> = [];
    const discover = makeDiscovery({ run: (_model, inputs, options) => {
    ...
    expect(requests[0]).toMatchObject({ inputs: { max_tool_calls: 1, max_output_tokens: 512, tools: [{ type: "web_search" }] }, options: { gateway: { retries: { maxAttempts: 1 }, skipCache: true } } });
  }));
});
