...
const Envelope = Schema.Struct({
  ...
  tools: Schema.Array(Schema.Struct({ type: Schema.Literal("web_search") })).check(Schema.isMinLength(1), Schema.isMaxLength(1)),
  usage: Schema.Struct({
    input_tokens: tokenCount,
    input_tokens_details: Schema.Struct({ cached_tokens: Schema.Literal(0) }),
  ...
  Schema.Union([
    Schema.Struct({
      action: Schema.Struct({
        query: text(2_000),
        sources: Schema.Array(Schema.Struct({ type: Schema.Literal("url"), url: text(4_096) })).check(Schema.isMaxLength(100)),
...
export const makeDiscovery = (binding: {
  readonly run: (model: typeof managedSearchPrice.model, inputs: Record<string, unknown>, options?: AiOptions) => Promise<unknown>;
}, gatewayId: string) =>
  Effect.fn("ManagedWebSearch.discover")(function* (
    ...
    const initial = dispatched ?? initialManagedSearchEvidence(yield* Effect.sync(() => crypto.randomUUID()));
    const inputs = {
    ...
    if (
      gatewayId.trim().length === 0 || query.length === 0 || query.length > limits.queryCharacters ||
      !Number.isInteger(limit) || limit < 1 || limit > limits.resultsPerSearch ||
    ...
    const response = yield* Effect.tryPromise({
      try: (signal) => binding.run(managedSearchPrice.model, inputs, {
        gateway: {
      ...
      catch: () => unavailable(initial, "The managed search attempt has unknown provider acceptance."),
    });
...
export const decodeResponse = (response: unknown, initial: ManagedSearchEvidence) =>
  Effect.gen(function* () {
    const envelope = yield* Schema.decodeUnknownEffect(Envelope)(response).pipe(
      Effect.mapError(() => unavailable(initial, "Managed search returned an invalid usage envelope.")),
    );
    const observed = {
      ...initial,
    ...
    const output = yield* Schema.decodeUnknownEffect(Output)(envelope.output).pipe(
      Effect.mapError(() => unavailable(observed, "Managed search returned invalid tool evidence.")),
    );
    const calls = output.filter((item) => item.type === "web_search_call");
    if (calls.length > 1 || calls.some((call) => call.status !== "completed" && call.status !== "failed")) {
      return yield* unavailable(observed, "Managed search did not finish within its one-call bound.");
    }
    ...
    const successfulSearches = executedSearches.filter((search) => search.outcome === "succeeded").length;
    ...
    const citations = output.flatMap((item) => item.type === "message" ? item.content.flatMap((content) => content.annotations) : []);
    return {
      evidence,
      results: calls.flatMap((call) => call.action.sources.map((source) => ({
        title: citations.find((citation) => citation.url === source.url)?.title ?? source.url.slice(0, 2_000),
        url: source.url,
      }))),
    };
  });
