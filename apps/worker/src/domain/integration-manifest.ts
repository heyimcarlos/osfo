...
export interface ResolvedIntegrationManifestOperation extends IntegrationManifestOperation {
  ...
  readonly decodeInput: (input: unknown) => Result.Result<unknown, IntegrationManifestValueInvalid>;
}
...
const CalendarCreatePrivateInput = Schema.Struct({
...
const CalendarUpdateEventInput = Schema.Struct({
  calendarId: boundedIdentity,
  change: boundedSummary,
  eventId: boundedIdentity,
});

const DriveGetMetadataInput = Schema.Struct({ fileId: boundedIdentity });
...
const inputDecoderFor = (
  ...
  switch (manifest.inputContract) {
    case "gmailFetchThreadV1":
      return manifestDecoder(GmailFetchThreadInput, manifest, "providerInput");
    case "gmailMessageV1":
      return manifestDecoder(GmailMessageInput, manifest, "providerInput");
    case "calendarListEventsV1":
      return manifestDecoder(CalendarListEventsInput, manifest, "providerInput");
    case "calendarCreatePrivateV1":
      return manifestDecoder(CalendarCreatePrivateInput, manifest, "providerInput");
    case "calendarUpdateEventV1":
      return manifestDecoder(CalendarUpdateEventInput, manifest, "providerInput");
    case "driveGetMetadataV1":
      return manifestDecoder(DriveGetMetadataInput, manifest, "providerInput");
    default:
