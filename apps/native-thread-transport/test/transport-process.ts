...
export const startTransportProcess = async (options: TransportProcessOptions) => {
  const child = spawn(process.execPath, ["dist/main.js"], {
    ...
    env: {
      ...
      OSFO_NATIVE_THREAD_TRANSPORT_PORT: "0",
  ...
  const port = await new Promise<number>((resolve, reject) => {
    ...
    child.stdout.on("data", (chunk: string) => {
      ...
      const match = output.match(
        /Native Thread Transport listening on http:\/\/127\.0\.0\.1:(\d+)/u,
      );
