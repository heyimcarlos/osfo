...
/** Interactive list of connected messaging channels. */
export function ConnectedChannels({
  ...
  return (
    <section className={surface} aria-labelledby="connected-channels-title">
      <h2 className="mb-3 text-base font-bold text-[#101936]" id="connected-channels-title">
        Connected Channels
      </h2>
      <div className="grid gap-2.5">
        {channels.map(({ color, icon: Icon, id, label }) => {
          ...
          return (
            <button
              aria-label={`${label}, ${primary ? "Primary" : "Connected"}`}
  ...
  return (
    <section className={surface} aria-labelledby="messaging-settings-title">
      ...
      <div className="mt-2.5 rounded-2xl border border-[#d4e0ef]/70 bg-white/58 p-4">
        <h3 className="font-semibold text-[#101936]">Primary Messaging Channel</h3>
        <p className="mt-0.5 text-xs text-[#65718a] sm:text-sm">
          Choose the default channel for conversations
        </p>
        <div
          aria-label="Primary Messaging Channel"
          className="mt-3 grid grid-cols-3 rounded-xl border border-[#d6e1ef] bg-[#edf3fa]/75 p-1"
          role="radiogroup"
        >
